...
def main() -> None:
    with tempfile.TemporaryDirectory(prefix="petrolab_rock_hardening_", ignore_cleanup_errors=True) as tmp:
        ...
        from petrolab.db import (
            add_dataset, create_project, get_or_create_library_project,
            link_dataset_to_project, unlink_dataset_from_project,
        )
        from petrolab.repositories.rock_repository import (
            create_rock, get_composition, get_rock, replace_isotopes,
            set_mineral_links, update_rock,
        )
        ...
        dataset_id = add_dataset(
            library_id, "R1 mica from library", "phlogopite", "mica.xlsx", "Sheet1",
            "library-sha", str(csv_path), 1, source_kind="managed_copy",
        )
        ...
        import_rocks_wide(
            ...
            project_id=project_id, name_column="Rock", chemistry_method="XRF",
            source="major.xlsx", on_conflict="update",
        )
        import_rocks_wide(
            pd.DataFrame({"Rock": ["R1"], "La ppm": [100.0], "Ce ppm": [200.0]}),
            project_id=project_id, name_column="Rock", chemistry_method="LA-ICP-MS",
            source="trace.xlsx", on_conflict="update",
        )
        ...
        _expect_value_error(
            lambda: import_rocks_wide(
                ...
                project_id=project_id, name_column="Rock", chemistry_method="XRF",
                source="bad.xlsx", on_conflict="update",
            ),
        ...
        del snapshot, orphaned, composition, after_failure, no_iso, incomplete_iso
        del grouped, export_snapshot, payload, chemistry_by_analyte, xlsx, workbook, exported_chemistry, exported_si
        gc.collect()
