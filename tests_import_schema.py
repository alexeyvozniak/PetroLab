...
result = import_linked_sheets(
    ...
    header_row=1,
    semantic_maps={
        "Standard": {"Sample": "Sample", "Generation": "Generation"},
        "Legacy": {"Sample": "Sample ID", "Generation": "Генерация"},
    },
)
...
assert legacy["FeO"].tolist() == [8.0, 9.0]
legacy_ids = legacy["_analysis_id"].tolist()
...
assert column_map["FeO"]["original"] == "FeOt"
...
# Reorder the legacy source columns and change values. Stored semantic roles must still apply.
book = openpyxl.load_workbook(workbook)
legacy_ws = book["Legacy"]
rows = list(legacy_ws.values)
headers = list(rows[0])
data_rows = [list(row) for row in rows[1:]]
new_order = [headers.index("Sample ID"), headers.index("SiO₂ (wt. %)"), headers.index("Генерация"), headers.index("FeOt"), headers.index("MgO")]
...
refresh_dataset_from_source(legacy_id)
refreshed = load_dataset_dataframe(legacy_id, include_meta=True)
assert refreshed["_analysis_id"].tolist() == legacy_ids
assert refreshed.loc[0, "Generation"] == "N-X2"
assert float(refreshed.loc[0, "SiO2"]) == 44.5
