...
from petrolab.partition_seed_germ import GERM_SELECTIONS, seed_germ_alkaline_models
from petrolab.partitioning import list_partition_models

ensure_storage()
assert all(path.is_file() for path in GERM_SELECTIONS.values())
created = seed_germ_alkaline_models()
assert created
models = list_partition_models()
assert len(models) == len(created)
assert {model["applicability"]["rock"] for model in models} >= {"Basanite", "Phonolite", "Carbonatite", "Lamprophyre"}
...
assert seed_germ_alkaline_models() == []
print("Built-in GERM alkaline library tests: OK")
