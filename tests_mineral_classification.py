...
near(sch_d["grt_X_sum"], 3.0); near(sch_d["grt_Y_sum"], 2.0); near(sch_d["grt_Z_sum"], 3.0)
near(sch_d["TiGrt_Sch"], 100.0); assert bool(sch_d["TiGrt_Fig5_applicable"]); assert sch_d["TiGrt_field"] == "Schorlomite"
...
near(mor_d["TiGrt_Mor"], 100.0); assert mor_d["TiGrt_field"] == "Morimotoite"
...
assert classified_mg[SPECIES_COL] == ""; assert "Mg-dominant morimotoite-analog" in classified_mg[NOTE_COL]

adr = pd.DataFrame([{
    "TiO2": 0.0, "apfu_Ca": 3.0, "apfu_Fe3": 2.0, "apfu_Si": 3.0,
    "Adr": 100.0, "Prp": 0.0, "Alm": 0.0, "Sps": 0.0, "Grs": 0.0, "Uv": 0.0,
}])
...
assert ol_r[SPECIES_COL] == "forsterite"; assert "Fo-dominant" in ol_r[FIELD_COL]

fsp = pd.DataFrame([{"SiO2": 68.74, "Al2O3": 19.44, "Na2O": 11.82}])
fsp_r = calculate_formula_safe(fsp, "feldspar", "fsp_8o").data.iloc[0]
assert fsp_r[SPECIES_COL] == "albite"; assert fsp_r[FIELD_COL] == "Albite"; assert "Gündüz" in fsp_r[METHOD_COL]

amp = pd.DataFrame([{
    "SiO2": 45.0, "Al2O3": 10.0, "MgO": 15.0, "CaO": 11.0,
    "Na2O": 3.0, "K2O": 1.0, "FeO": 12.0,
}])
amp_r = calculate_formula_safe(amp, "amphibole", "amp_ima2012_23o").data.iloc[0]
assert amp_r[SPECIES_COL] == ""; assert "formal species not yet assigned" in amp_r[FIELD_COL]
...
assert apt_r[SPECIES_COL] == ""; assert "fluorapatite" in apt_r[FIELD_COL]; assert "X-anion" in apt_r[LEVEL_COL]

# Missing halogen measurements must never be promoted to a hydroxylapatite field.
apt_missing = pd.DataFrame([{"P2O5": 42.0, "CaO": 55.0}])
apt_missing_r = calculate_formula_safe(apt_missing, "apatite", "ap_ketcham25").data.iloc[0]
assert "hydroxylapatite" not in apt_missing_r[FIELD_COL].lower()
assert "unresolved" in apt_missing_r[FIELD_COL].lower()
assert apt_missing_r[LEVEL_COL] == "insufficient X-anion data"
