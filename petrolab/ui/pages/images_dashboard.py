...
def _entry_state(dataset_id: int, token: str) -> tuple[str, bool]:
    ...
    return prefix, assignment_error(prefix, scope_type) is None
...
def _assignment(prefix: str, filename: str, data: bytes) -> ImageAssignment | None:
    ...
    if scope_type == "skip":
        return None
    if assignment_error(prefix, scope_type):
    ...
    if scope_type == SCOPE_ANALYSIS:
        ...
        scope = ImageScope(
            SCOPE_FIELD,
            scope_column=st.session_state.get(f"{prefix}_field_column", ""),
            scope_value=st.session_state.get(f"{prefix}_field_value", ""),
        )
    else:
    ...
    return ImageAssignment(
        ...
        scope,
        st.session_state.get(f"{prefix}_kind", IMAGE_KINDS[0]),
...
def _wizard(project_id: int, dataset: dict, dataframe: pd.DataFrame) -> None:
    ...
    files = st.file_uploader(
        ...
        key=f"image_batch_upload_{dataset_id}_{epoch}",
    )
    if not files:
    ...
    entries = [
        (file.name, file.getvalue(), f"{index}_{hashlib.sha256(file.getvalue()).hexdigest()[:10]}")
    ...
    status_items = []
    for item_index, (name, _, item_token) in enumerate(entries):
        _, ready = _entry_state(dataset_id, item_token)
        marker = "→" if item_index == index else ("✓" if ready and item_index < index else "○")
        status_items.append(
            (f"{marker} {name}", "success" if marker == "✓" else "accent" if marker == "→" else "neutral")
        )
    ...
    previous_col, next_col, review_col = st.columns([1, 1, 2])
    if previous_col.button("← Назад", disabled=index == 0, width="stretch"):
        st.session_state[index_key] = index - 1
        st.rerun()
    if next_col.button("Далее →", disabled=index == len(entries) - 1, type="primary", width="stretch"):
    ...
    for name, raw, item_token in entries:
        ...
        try:
            ...
            errors.append(name)
    if review_col.button("Проверить и сохранить пачку", width="stretch"):
        if errors:
...
def _repair_detached(asset: dict, dataframe: pd.DataFrame) -> None:
    ...
    with st.expander("Восстановить привязку", expanded=False):
        query = st.text_input(
            "Поиск точки",
            placeholder="Sample, Grain, Point, Generation…",
        ...
        labels = analysis_id_labels(candidates.head(limit))
        ...
        if st.button(
            ...
            key=f"repair_image_save_{asset_id}",
            width="stretch",
        ):
            try:
...
def render_images_dashboard_page() -> None:
    render_page_header(
        "Изображения",
        "Связывайте BSE, EDS, карты и фотографии с аналитическими точками без потери контекста.",
        eyebrow="Материалы",
    )
