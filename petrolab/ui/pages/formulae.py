...
def _render_classification_summary(result: pd.DataFrame) -> None:
    ...
    st.dataframe(result[_identity_columns(result) + columns].head(1000), width="stretch", hide_index=True, height=min(420, 42 + 35 * min(len(result), 10)))
...
def render_formulae_page() -> None:
    ...
    if not datasets:
        st.info("В активном проекте пока нет анализов."); return
    ...
    if not methods:
        st.warning("Для этого минерала пока нет валидированного минералоспецифического пересчёта."); return

    method_map = {method.id: method for method in methods}
    method_id = st.selectbox("Метод", list(method_map), format_func=lambda value: method_map[value].title_ru, key="formula_method")
    ...
    with st.expander("Метод, допущения и источники", expanded=False):
        ...
        if method.warning_ru: st.warning(method.warning_ru)
        for reference in method.references: st.caption("• " + reference)

    source = load_dataset_dataframe(dataset_id, include_meta=True)
    if source.empty:
        st.info("В наборе нет аналитических строк."); return
    try:
        result = calculate_formula_safe(source, chosen["mineral_key"], method.id)
    except Exception as exc:
        st.error(f"Пересчёт остановлен: {exc}"); return
    ...
    render_badges([
        ...
        (
            "✓ Актуален" if status.has_active_formula and status.method_id == method.id and status.stale_rows == 0
            else f"↻ Пересчитать: {status.stale_rows}" if status.stale_rows
            else "○ Не сохранён",
            "success" if status.has_active_formula and status.method_id == method.id and status.stale_rows == 0 else "warning" if status.stale_rows else "neutral",
        ),
    ])
    ...
    if not derived:
        st.warning("Метод не создал новых расчётных колонок для этого набора."); return

    _render_classification_summary(result.data)
    render_section_header("Результаты", "Предпросмотр первых 1000 строк")
    ...
    if st.button("Сохранить расчёт в рабочую базу", type="primary", key="save_formula_results", width="stretch"):
        saved = save_formula_results(
            dataset_id=dataset_id, mineral_key=chosen["mineral_key"], method_id=method.id,
            method_title=method.title_ru, source_dataframe=source, result_dataframe=result.data,
        )
