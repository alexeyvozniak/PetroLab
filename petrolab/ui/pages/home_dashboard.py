...
def render_home_dashboard_page() -> None:
    ...
    render_section_header("Продолжить работу", "Обычный путь — через «Рабочий процесс»; любой модуль можно открыть напрямую")
    labels = [
        ("01 · Рабочий процесс", "Файл → фазы → формулы → фото/шлиф → график", "workflow"),
        ("02 · Новые анализы", "Excel / CSV и обновление источников", "sources"),
