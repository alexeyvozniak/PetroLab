...
def _render_project_checklist(project_id: int, datasets: list[dict]) -> None:
    """Personal, reversible next actions which never alter analytical data."""
    render_section_header(
        "Следующие действия",
        "Личный список активного проекта. Выполненные пункты скрываются, но остаются в журнале.",
    )
    ...
    with st.form("home_project_checklist_form", clear_on_submit=True):
        title = st.text_input("Что нужно сделать?", placeholder="Например: разобрать LA-ICP-MS для 19 ТР-1")
        details = st.expander("Детали задачи", expanded=False)
        with details:
            left, right = st.columns(2)
            dataset_id = left.selectbox(
                "Связать с набором", [None, *by_dataset_id],
            ...
            note = st.text_input("Пояснение", placeholder="Что именно проверить или подготовить")
        submitted = st.form_submit_button("Добавить действие", type="primary")
    if submitted:
        try:
            create_project_checklist_item(
                project_id, title=title, note=note,
    ...
    if not active:
        st.caption("Открытых действий нет. Добавьте следующий шаг, пока он не потерялся среди файлов и измерений.")
    for item in active:
        item_id = int(item["id"])
        tick, body, action = st.columns([0.55, 3.25, 1.1], vertical_alignment="center")
        with tick:
            done = st.checkbox("Готово", key=f"home_project_task_done_{item_id}")
        with body:
            st.markdown(f"**{item['title']}**")
            details = [f"Набор: {item['dataset_name']}" if item.get("dataset_name") else "", str(item.get("note") or "")]
            if any(details):
        ...
        with st.expander(f"Выполнено · {len(completed)}", expanded=False):
            for item in completed:
                ...
                with left:
                    st.markdown(f"~~{item['title']}~~")
                with right:
                    if st.button("Вернуть", key=f"home_project_task_restore_{item['id']}", width="stretch"):
                        set_project_checklist_item_completed(project_id, int(item["id"]), completed=False)
                        st.rerun()
