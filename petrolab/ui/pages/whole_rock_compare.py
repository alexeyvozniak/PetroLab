...
def _focus_ids(project_id: int, dataframe: pd.DataFrame) -> list[int]:
    ...
    available = set(pd.to_numeric(dataframe.get("_rock_id", pd.Series(dtype=float)), errors="coerce").dropna().astype(int).tolist())
    requested = []
...
def _apply_plot_groups(dataframe: pd.DataFrame, focus_ids: list[int]) -> pd.DataFrame:
    ...
    if focus_ids and "_rock_id" in result.columns:
        focus_set = set(int(value) for value in focus_ids)
        ids = pd.to_numeric(result["_rock_id"], errors="coerce")
        mask = ids.isin(focus_set)
        names = result["Rock"].astype(str) if "Rock" in result.columns else ids.astype("Int64").astype(str)
...
def _shared_styles(dataframe: pd.DataFrame, key: str, group_column: str) -> dict:
    ...
    with st.expander("Стили серий", expanded=True):
        st.caption(
            "Фокусная порода имеет отдельную plot-group, но её настоящий источник/provenance не меняется. Для литературных серий можно уменьшить Alpha или показать поле."
        )
...
def _figure_exports(fig, stem: str, key: str) -> None:
    c1, c2 = st.columns(2)
    c1.download_button("SVG", figure_svg_bytes(fig), file_name=f"{stem}.svg", mime="image/svg+xml", width="stretch", key=f"{key}_svg")
    c2.download_button("PNG 600 dpi", figure_png_bytes(fig, 600), file_name=f"{stem}.png", mime="image/png", width="stretch", key=f"{key}_png")
...
def _xy_tab(dataframe: pd.DataFrame, group_column: str, key: str) -> None:
    ...
    figure = build_scatter(
        ...
        x,
        y,
        group_column if group_column in dataframe.columns else None,
        x_label=x_label,
        y_label=y_label,
        title=title,
...
def render_whole_rock_compare_page() -> None:
    ...
    render_page_header(
        ...
        "Whole-rock данные работают как исследовательская выборка: источники можно включать и выключать, фокусную породу держать отдельной серией, а отдельные образцы скрывать без удаления.",
        eyebrow="Породы",
    ...
    if focus_ids:
        ...
        ].astype(str).tolist() if "Rock" in dataframe.columns else list(map(str, focus_ids))
    ...
    if dataframe.empty:
        ...
        return
    visible_focus_ids = [
        rock_id for rock_id in focus_ids
        if rock_id in set(pd.to_numeric(dataframe["_rock_id"], errors="coerce").dropna().astype(int).tolist())
    ] if "_rock_id" in dataframe.columns else []
