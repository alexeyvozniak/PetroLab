...
from petrolab.ternary_presets import TernaryPreset, apply_preset_projection, available_ternary_presets


_NORMALIZATION_WIDGET = {
    "auto": "Авто",
    "normalize": "Нормировать каждую строку к 100%",
    "already": "Считать значения уже согласованными",
}
...
def _custom_selection(dataframe: pd.DataFrame, recipe: dict) -> TernarySelection:
    ...
    return TernarySelection(dataframe, "Своя диаграмма", "", a_col, b_col, c_col, a_label, b_label, c_label, None, False)
...
def _editable_overlay_labels(overlay: TernaryOverlay, recipe: dict) -> TernaryOverlay:
    ...
    show_labels = st.checkbox(
        ...
        value=bool(recipe.get("show_classification_labels", True)), key="ternary_show_field_labels",
    )
    if not show_labels:
    ...
    with st.expander("Подписи классификационных полей", expanded=False):
        ...
        for index, label in enumerate(overlay.labels):
            text = st.text_input(
                f"Поле {index + 1}", value=str(saved.get(str(index), label.text)),
...
def _preset_selection(dataframe: pd.DataFrame, recipe: dict) -> TernarySelection | None:
    # Availability must inspect real rows/minerals, not only the union of columns in a
    # mixed unified dataframe.
    available = available_ternary_presets(dataframe)
    if not available:
        return None
    labels = {preset.title_ru: preset for preset in available}
    saved_id = recipe.get("ternary_preset_id")
    default_label = next((label for label, preset in labels.items() if preset.preset_id == saved_id), next(iter(labels)))
    chosen = st.selectbox("Минералогический шаблон", list(labels), index=list(labels).index(default_label), key="ternary_preset")
    preset: TernaryPreset = labels[chosen]
    st.caption(preset.description_ru)

    normalization = str(recipe.get("ternary_normalization", preset.normalization)) if saved_id == preset.preset_id else str(preset.normalization)
    signature = f"{preset.preset_id}|{normalization}"
    if st.session_state.get("_ternary_normalization_signature") != signature:
        st.session_state["ternary_normalization"] = _NORMALIZATION_WIDGET.get(normalization, "Авто")
        st.session_state["_ternary_normalization_signature"] = signature
    ...
    with st.expander("Подписи вершин", expanded=False):
        l1, l2, l3 = st.columns(3)
        a_label = l1.text_input("Левая вершина", value=str(saved.get("a_label", preset.a_label)), key="ternary_preset_a_label")
        b_label = l2.text_input("Правая вершина", value=str(saved.get("b_label", preset.b_label)), key="ternary_preset_b_label")
        c_label = l3.text_input("Верхняя вершина", value=str(saved.get("c_label", preset.c_label)), key="ternary_preset_c_label")
    ...
    if overlay is not None:
        show_overlay = st.checkbox(
            ...
            value=bool(recipe.get("show_classification_overlay", True)), key="ternary_show_overlay",
        )
        if show_overlay:
    ...
    return TernarySelection(
        projected, "Шаблон", preset.preset_id,
        components[0], components[1], components[2],
        a_label, b_label, c_label, overlay, show_overlay,
    )
...
def render_ternary_selection(dataframe: pd.DataFrame, recipe: dict) -> TernarySelection:
    ...
    mode = st.radio(
        "Режим ternary", ["Шаблон", "Своя диаграмма"], horizontal=True,
        index=0 if mode_default == "Шаблон" else 1, key="ternary_mode",
    )
    if mode == "Шаблон":
        ...
        if selection is not None:
            return selection
        st.warning(
            "В текущих строках нет валидных компонентов готового минералогического шаблона. "
            "Сохраните соответствующий formula-derived слой или выберите свои три компонента."
        )
