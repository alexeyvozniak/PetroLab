...
MINERAL_RECOGNITION_RULESET_VERSION = "2026.08.5"
...
def score_candidates(row: Mapping[str, Any]) -> dict[str, MineralCandidate]:
    ...
    if _between(tio2, 20, 45) and _between(cao, 18, 35) and _between(sio2, 20, 42):
    ...
    carbonate_gate = (
        (not np.isfinite(sio2) or sio2 < 8)
        and (not np.isfinite(p2o5) or p2o5 < 5)
        and (not np.isfinite(so3) or so3 < 8)
        and (not np.isfinite(tio2) or tio2 < 5)
        and (not np.isfinite(cr2o3) or cr2o3 < 5)
        and (not np.isfinite(al2o3) or al2o3 < 8)
        and (not np.isfinite(zro2) or zro2 < 5)
    )
    if carbonate_gate:
        ca_f, mg_f, fe_f, mn_f, sr_f = _fraction(cao, mgo, feo, mno, sro)
        if ca_f > 0.80 and divalent > 35:
            add("Ca-carbonate", 8, "Ca-dominant, Si/P/S/Ti/Cr/Al-poor composition")
        if ca_f > 0.30 and mg_f > 0.25 and divalent > 35:
            add("Ca-Mg carbonate", 8, "Ca-Mg-dominant carbonate-like cation balance")
        ...
        if mg_f > 0.75 and divalent > 30:
            add("Mg-carbonate", 8, "Mg-dominant carbonate-like cation balance")
        if fe_f > 0.70 and divalent > 30:
            add("Fe-carbonate", 8, "Fe-dominant carbonate-like cation balance")
        if mn_f > 0.65 and divalent > 30:
            add("Mn-carbonate", 8, "Mn-dominant carbonate-like cation balance")
        if sr_f > 0.65 and divalent > 25:
            add("Sr-carbonate", 9, "Sr-dominant carbonate-like cation balance")
    ...
    # Analcime is a hydrous Na-Al-Si framework phase. In volatile-free normalized EPMA totals it
    # overlaps Na feldspar, so high Al2O3 and sub-albite SiO2 are required before it can outrank
    # plagioclase. This deliberately avoids trying to distinguish hydration state from EPMA alone.
    if (
        _between(sio2, 58, 66.5)
        and _between(al2o3, 22, 30)
        and _between(na2o, 7, 16.5)
        and (not np.isfinite(k2o) or k2o < 2)
        and (not np.isfinite(cao) or cao < 2)
        and mafic < 5
    ):
        add("analcime", 11, "Na-Al-Si framework chemistry with analcime-like Si/Al")
    ...
    na_k_total = (na2o if np.isfinite(na2o) else 0.0) + (k2o if np.isfinite(k2o) else 0.0)
    k_alkali_fraction = (k2o / na_k_total) if np.isfinite(k2o) and na_k_total > 0 else 0.0
    if _between(sio2, 30, 52) and _gt(k2o, 5) and k_alkali_fraction >= 0.60 and _between(al2o3, 0.3, 35):
        if mafic > 10:
            add("trioctahedral mica", 9, "K-dominant Mg-Fe mica chemistry")
        elif al2o3 > 20:
            add("dioctahedral mica", 8, "K-dominant Al-rich mica chemistry")
    ...
    return {
        target: MineralCandidate(target=target, score=float(score), reasons=tuple(reasons))
