...
Checklist items describe a researcher's intent, not a mutable property of an
analysis. Completing an item therefore never edits source measurements or QC.
"""
...
def create_project_checklist_item(
    ...
    *,
    ...
    """Add a personal next step without changing an analytical record."""
...
def list_project_checklist_items(project_id: int, *, completed: bool = False, limit: int | None = None) -> list[dict]:
    """Return active items or compact completed history for one project."""
    ensure_project_checklist_schema()
