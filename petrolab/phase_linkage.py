...
def _phase_from_linked(con, linked_ids: set[str]) -> tuple[str, str, bool]:
    ...
    # A human-confirmed exact phase is stronger than a broad dataset module.
    if _table_exists(con, "analysis_annotations"):
        rows = con.execute(
            f"""SELECT analysis_id, value FROM analysis_annotations
                WHERE namespace='phase' AND key='confirmed_phase'
                  AND analysis_id IN ({marks}) AND TRIM(value)<>''""",
            list(linked_ids),
        ).fetchall()
        labels = {str(row["value"]).strip() for row in rows if str(row["value"]).strip()}
        if len(labels) == 1:
            return next(iter(labels)), "та же физическая метка; фаза подтверждена у связанной строки", False
        if len(labels) > 1:
            return "", "связанные анализы имеют разные подтверждённые фазы", True
    ...
    if len(labels) == 1:
        return next(iter(labels)), "та же физическая метка; минералогический модуль связанной строки", False
    if len(labels) > 1:
