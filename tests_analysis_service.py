...
def main() -> None:
    with tempfile.TemporaryDirectory(prefix="petrolab_analysis_service_") as tmp:
        ...
        file_a = base / "a.xlsx"
        file_b = base / "b.xlsx"
        _write_workbook(file_a, {"Mica": frame_a})
        _write_workbook(file_b, {"Mica": frame_b})
        ...
        assert wb["Mica"]["B2"].value == 40.0
        wb.close()
        wb = openpyxl.load_workbook(file_b, data_only=True)
        assert wb["Mica"]["B2"].value == 42.0
        wb.close()
        ...
        assert len(valid_result.backup_paths) == 2
        assert all(Path(path).exists() for path in valid_result.backup_paths)

        wb = openpyxl.load_workbook(file_a, data_only=True)
        assert wb["Mica"]["B2"].value == 44.4
        wb.close()
        wb = openpyxl.load_workbook(file_b, data_only=True)
        assert wb["Mica"]["B2"].value == 46.6
        wb.close()
        ...
        assert len(one_file_result.backup_paths) == 1

        wb = openpyxl.load_workbook(multi, data_only=True)
        assert wb["Core"]["B2"].value == 48.1
        assert wb["Rim"]["B2"].value == 49.2
        wb.close()
