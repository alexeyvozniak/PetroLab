...
def main() -> None:
    ...
    dataset_id = add_dataset(project_id, "LA-ICP-MS · 19 ТР-1", "phlogopite", "19tr1.xlsx", "LA", "hash", str(Path(_TMP.name) / "rows.csv"), 3)
    item_id = create_project_checklist_item(
        project_id, title="Разобрать LA-ICP-MS для 19 ТР-1",
        note="Проверить названия фаз и Generation", dataset_id=dataset_id, target_route="analyses",
    )
    ...
    assert set_project_checklist_item_completed(project_id, item_id, completed=True)
    assert list_project_checklist_items(project_id) == []
    assert list_project_checklist_items(project_id, completed=True)[0]["completed_at"]
    assert set_project_checklist_item_completed(project_id, item_id, completed=False)
    try:
