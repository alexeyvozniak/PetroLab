...
Интерфейс v0.11 построен как спокойный светлый научный дашборд: активный проект всегда виден в боковой панели, основные рабочие разделы открываются одним кликом, а результат — таблица, график или изображение — остаётся главным объектом экрана.
...
GitHub Actions на Windows проверяет BAT/CRLF, архитектурные границы, импорт и синхронизацию, v0.11 scientific/data-integrity regressions, минералогические формулы, classification overlays, whole-rock/statistics modules, responsive UI, реальные browser viewports и открытие страниц Streamlit через AppTest.
