...
def main() -> None:
    with tempfile.TemporaryDirectory(prefix="petrolab_rock_workspace_", ignore_cleanup_errors=True) as tmp:
        root = Path(tmp)
        ...
        composition = {
            "SiO2": 44.0, "TiO2": 2.0, "Al2O3": 12.0, "FeOt": 10.0,
            "MnO": 0.2, "MgO": 12.0, "CaO": 11.0, "Na2O": 2.5,
            "K2O": 3.0, "P2O5": 0.8, "La [µg/g]": 120.0, "Ce [µg/g]": 240.0,
        }
        units = {
            **{key: "wt.%" for key in ["SiO2", "TiO2", "Al2O3", "FeOt", "MnO", "MgO", "CaO", "Na2O", "K2O", "P2O5"]},
            "La [µg/g]": "µg/g", "Ce [µg/g]": "µg/g",
        }
        replace_composition(
            rock_id, composition, units=units, method="XRF + ICP-MS", source="Vozniak et al., own data"
        )
        replace_isotopes(
            rock_id,
            pd.DataFrame([{
                "system": "Sr", "ratio_name": "87Sr/86Sr", "value": 0.7032,
                "uncertainty": 0.00002, "initial_value": 0.7029, "age_ma_used": 380.0,
                "method": "TIMS", "laboratory": "IGEM", "source": "own", "notes": "",
            }]),
        )

        dataset_csv = root / "mica.csv"
        mineral_rows = pd.DataFrame({
            "Sample": ["KIV-2", "KIV-2"], "Point": ["P1", "P2"],
            "SiO2": [39.0, 39.5], "MgO": [20.0, 19.5],
        })
        mineral_rows.to_csv(dataset_csv, index=False)
        dataset_id = add_dataset(
            project_id, "KIV-2 phlogopite", "phlogopite", "mica.xlsx", "Sheet1",
            "rock-workspace-sha", str(dataset_csv), len(mineral_rows), source_kind="managed_copy",
        )
        ...
        replace_composition(
            incomplete_id, {"SiO2": 50.0, "MgO": 8.0},
        ...
        del snapshot, majors, traces, incomplete
        gc.collect()
